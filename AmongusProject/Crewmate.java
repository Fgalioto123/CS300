...
/**
 * Provided Crewmate object for P2. This draggable object can represent a normal
 * or impostor Crewmate
 * of several potential colors. When an impostor is dragged over a normal
 * Crewmate, the normal one
...
public class Crewmate {
  ...
  // constructors
  /**
   * Basic constructor to create your first normal Crewmate. This Crewmate appears
   * at the center of the
   * game window.
   * 
   * @param color an integer representing the color index of this Crewmate, in
   *              {1,2,3}
   */
  ...
  /**
   * Advanced constructor, which allows creation of Crewmate at specified (x,y)
   * locations and also
   * impostor Crewmate.
   * 
   * @param color      an integer representing the color index of this Crewmate,
   *                   in {1,2,3}
  ...
  public Crewmate(int color, float x, float y, boolean isImpostor) {
    if (color < 1 || color > 3)
      throw new IllegalArgumentException("Color must be 1, 2, or 3;" +
  ...
  // callback
  /**
   * Causes this Crewmate to be drawn to the game window. This method should be
   * called in your
  ...
  public void draw() {
    ...
    if (IS_IMPOSTOR) {
      Utility.reverseImage((isDead ? this.deadImage : this.image), x, y);
    } else {
      Utility.image((isDead ? this.deadImage : this.image), x, y);
    }
  }
  ...
  public boolean isImpostor() {
    return this.IS_IMPOSTOR;
  }

  /**
   * Provides the current image (dead or alive) of this Crewmate in the correct
   * color
   * 
   * @return a reference to the correct image for this Crewmate given its state
   */
  public PImage image() {
    return isDead ? this.deadImage : this.image;
  }
  ...
  /**
   * Sets the dragging status of this Crewmate to true and begins recording mouse
   * movements
   */
  ...
  /**
   * After calling this method, this Crewmate will be dead. This action is not
   * reversible.
   */
  public void unalive() {
    if (!IS_IMPOSTOR)
