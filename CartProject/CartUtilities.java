...
/**
 * This class is used with a cart array to add, remove, find the location of,
 * and get the cost of an
 * item. It is also used to find the total cost of all items. It could be used
 * by an online shopping
 * company.
 */
public class CartUtilities {
   ...
   * @param cartSize    - It expects a size of valid items in the cart which is a
   *                    integer
   * @param description - it also expects the name of the item it is looking for
   *                    in the cart
  ...
  public static int indexOfItem(String[][] cart, int cartSize, String description) {
    if (cart == null || cart[0] == null || cart[0][0] == null) {
      return -1;
    }
    // a for loop that goes through the array
    for (int i = 0; i < cartSize; i++) {
      // checks if the name of the item is the same as the description. If it is that
      // means that the
      // items are the same and that i is the index of the item so it returns it.
      if (cart[i][0].equals(description) && !(cart[i][0].equals(null))) {
        return i;
      }
    }
  ...
  /**
   * this method takes an item name and adds it to the cart by either creating a
   * new item in the
   * cart of adding one to the amount of a previous item
   * 
   ...
   * @return it returns the cartsize which could be larger or the same depending
   *         on if the item is
   *         new or not
   */
  public static int addItemToCart(String[][] cart, int cartSize, String description) {
    ...
    if (index != -1) {
      ...
      // if it does not exists then it checks to make sure there is space and if there
      // is it adds
      // the
      // item to the end
    } else if (cart[cartSize] == null) {
  ...
  /**
   * this method removes an item from the cart if the index given is valid and
   * shifts the other
   * items down if the index was somewhere before the last valid item
   * 
   ...
   * @return returns the the cartsize which is either lower or the same depending
   *         if the index was
   *         valid or not
   */
  public static int removeItemFromCart(String[][] cart, int cartSize, int index) {
    ...
    // it starts at the index given and makes sure it is less then cartsize because
    // if it is not
    // then that would mean the index given is not a real item but a null
    for (int i = index; i < cartSize; i++) {
      // if the index is the last item then just turn it to null because there is
      // nothing after it
      // to move back in the array
      if (i == cartSize - 1) {
        cart[i] = null;
      } else {
        // if there are valid items in front then it would continue the loop and keep
        // settings the
        // ones in front of it to the previous one so there is no gap in the cart array
        cart[i][0] = cart[i + 1][0];
        cart[i][1] = cart[i + 1][1];
      }
    }
   ...
   * @param costs       - expects a valid costs integer array and it to align to
   *                    the inventory array
   * @param description - expects the name of the item it is finding the cost for
   * @return it returns the cost of the item if it is found and if not then
   *         returns 0
   */
  public static int getCostOfItem(String[] inventory, int[] costs, String description) {
    ...
    int cost = -1;
    // goes through the inventory until it finds the item and when it does it uses
    // that i position
  ...
  /**
   * this methods gets the total cost of all the items in the cart and takes
   ...
   * @param inventory - expects the inventroy of the items if the inventory has
   *                  any items
  ...
  public static int getTotalCost(String[][] cart, int cartSize, String[] inventory, int[] costs) {
    if (cart == null || cart[0] == null || cart[0][0] == null || inventory == null
        || costs == null) {
      return cartSize;
    }
    // sets the totalcost initially to 0
    int totalCost = 0;
    // goes through each valid item of the cart and sets the cost using the previous
    // method. It then
    // adds the cost multiplied by the amount (which is found using Integer.parse
    // int) to total
    // costs
    for (int i = 0; i < cartSize; i++) {
