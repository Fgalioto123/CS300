...
/**
 * This class is used with a cart array to add, remove, find the location of, and get the cost of 
 * an item. It is also used to find the total cost of all items. It could be used by an online 
 * shopping company.
 */
public class CartUtilities {                  
  ...
  public static int removeItemFromCart(String[][] cart, int cartSize, int index) {
    ...
    for(int i = index; i < cartSize; i++) {
      ...
      // ones infront of it to the previous one so there is no gap in the cart array
      } else {
        cart[i][0] = cart[i+1][0];
        cart[i][1] = cart[i+1][1];
      }
