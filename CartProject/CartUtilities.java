...
/**
 * This class is used with a cart array to add, remove, find the location of, and get the cost of an
 * item. It is also used to find the total cost of all items. It could be used by an online shopping
 * company.
 */
public class CartUtilities {
  ...
  /**
   * this method takes an item name and adds it to the cart by either creating a new item in the
   ...
   * @return it returns the cartsize which could be larger or the same depending on if the item is
   *         new or not
   */
  public static int addItemToCart(String[][] cart, int cartSize, String description) {
    ...
    if (index != -1) {
      ...
      // if it does not exists then it checks to make sure there is space and if there is it adds
      // the
      // item to the end
  ...
  /**
   * this method removes an item from the cart if the index given is valid and shifts the other
  ...
  /**
   * this methods gets the total cost of all the items in the cart and takes account if there are
