...
/**
 * Contains testing methods for each method in CartUtilities. Each testing
 * method returns true when
 * all its test cases pass, otherwise false.
 */
public class CartUtilitiesTester {
  ...
  public static boolean testIndexOfItem() {
    ...
    // this test case tests if the methods runs correctly with bread being the
    ...
    /*
     * the edge case is if they mistype the description of the word and if it ever
     * returns a valid
     * index then return false because it should always return -1.(I am not sure if
     * this is
     * considered an edge case and if this is wrong and I am still struggling with
     * them on the next
     * program I will most likely come in for office hours next week.)
     */
  ...
  public static boolean testAddItemToCart() {
    // String[][] cart = {{"bread", "1"}, {"milk", "2"}, {"null", "null"}};
    String[][] cart = { { "bread", "1" }, { "milk", "2" }, null };
    String[][] cart2 = null;
    int cartSize = 2;
    ...
    // used to test a null cart;
    if (CartUtilities.addItemToCart(cart2, cartSize, "cheese") != cartSize) {
      return false;
    }

    // checks to see if the cartsize did change this time when the item was already
    // in there then
    // return false
    if (CartUtilities.addItemToCart(cart, cartSize, "cheese") != cartSize) {
      return false;
  ...
  public static boolean testRemoveItemFromCart() {
    ...
    // checks that cartsize is not two and if it is not two then return false
    // because if it is not
    // two then that means that cartsize did not go down and the item was not
    // removed.
    if (CartUtilities.removeItemFromCart(cart, cartSize, 1) != cartSize - 1) {
      return false;
    }

    // checks to see if it removes a null and counts that as the cartsize shrinking
    // which it should
  ...
  public static boolean testGetCostOfItem() {
    ...
    // makes sure that when you get the cost of cheese it is four and if it is not
    // then return false
    ...
    /*
     * edge case where the user could ender a capital C instead of a lowercase one
     * and if the method
     * returns four then return false because it should never return four because it
     * is not the
     * correct item.
     */
    if (CartUtilities.getCostOfItem(inventory, costs, "Cheese") == 4) {
  ...
  public static boolean testGetTotalCost() {
    ...
    // checks to see if the total costs of all the items is 12 and if it is not then
    // return false
    ...
    // now does the same method but this time the inventory doesn't have cheese so
    // it makes sure the
    // answer is eight and if it is not return false because the two items add up to
    // eight.
    ...
    /*
     * this one uses a different cart where there is four bread so it checks to make
     * sure the method
     * can calculate multiple of an item. Checks to make sure it returns 27 cause
     * that is the total
